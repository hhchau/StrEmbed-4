...
sub tk_mainloop {
    ...
    my $label = $mw -> title("Structure Embedding version 4 (StrEmbed-4)");
...
#        -height => 15,
    ...
    #    -menuitems => [
    #        [ 'command' => "~Save STEP file", -command => sub { &file_save } ],
    #        [ 'command' => "~Exit", -command => sub { exit } ],
    #    ]
    #) -> pack(
    #    -anchor => 'nw',
...
sub tk_display_order {
    my @labels = reverse ("element_highlighted",
                          "is_covered_by_highlighted",
                          "element",
                          "is_covered_by");  # from front to back
...
sub file_save {
    ...
    # $menu_01 -> configure(-state => 'disabled') if $file;
}
...
sub tk_embed_height_n {
    ### consider 2^n hypercube when height = 1
