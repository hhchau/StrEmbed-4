...
sub hypercube_corresponding_to_step_file {
    $max = shift;   # setting $max for the first and only time in this hypercube module
...
sub XXX_print_hypercube_out_of_order {
    print "(", &name($max), "\n";
    for my $e (0..2**$max-1) {
...
sub XXX_print_hypercube_by_level {
    print "(", &name($max), "\n";
    foreach my $r (0..$max) {
...
sub XXX_name {
    my $i = shift || $max;
    if    ($i == 0) {return "point_" . $i;}
    elsif ($i == 1) {return "line_" . $i;}
    elsif ($i == 2) {return "square_" . $i;}
    elsif ($i == 3) {return "cube_" . $i;}
    elsif ($i == 4) {return "tessaract_" . $i;}
    else            {return "hypercube_" . $i;}
}
