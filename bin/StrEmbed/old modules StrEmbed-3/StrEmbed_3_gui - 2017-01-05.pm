...
sub tk_embed_height_1 {
    ...
    if (&tk_check_if_all_exists(@siblings)) {
        ...
        my $meet = &meet(@bundle_of_ids);
        # $array_lookup_id_to_hash{$meet}{label} = $parent;
        # $lookup_label_to_id{$parent} = $meet;
        print "bundle @siblings (@bundle_of_ids) -> $parent ($meet)\n";
    } else {
        my $swap = shift @list;
        push @list, $swap;
        goto SECOND_ATTEMPT;
