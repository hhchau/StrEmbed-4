...
#        -height => 15,
    ...
    my $menu_2 = $pm -> Menubutton( -text => "STEP",
        -menuitems => [
            ...
            # [ 'command' => "print assembly",      -command => sub { &step_print_assembly } ],
            "-",
            # [ 'command' => "produce tree",        -command => sub { &step_produce_tree } ],
            # [ 'command' => "print tree",          -command => sub { &step_print_tree } ],
            [ 'command' => "insert tree items",     -command => sub { &tk_insert_tree_items } ],
            "-",
            [ 'command' => "Something else",      -command => sub { } ],
        ]
    ) -> pack(
        -anchor => 'nw',
        -side => 'left',
