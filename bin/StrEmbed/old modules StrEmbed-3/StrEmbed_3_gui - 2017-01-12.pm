...
#        -height => 15,
    ...
    my $menu_01 = $pm -> Menubutton( -text => "Tree editor",
        -menuitems => [
            [ 'command' => "~Exit", -command => sub { exit } ],
        ]
    ) -> pack(
...
sub tk_display_order {
    $c -> raise("is_covered_by_highlighted", "is_covered_by");
    $c -> raise("is_covered_by_highlighted", "element");
    $c -> raise("element_highlighted", "element");
}
...
sub tk_insert_tree_items {
    ...
    foreach my $ref_list (@items) {
        ...
        my $entry= $tree -> add($item, -text => $name);
    }
    $tree -> autosetmode;
