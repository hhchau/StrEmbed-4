...
sub step_produce_parent_child_pairs {
    ### i/p - cdsr-rr-sr
    ###     - cdsr-pds-nano-pd-pdfwss-p
    ### o/p - @parent_child_pair
    ###     - %atoms_available
    # print "produce parent child pairs\n";
    @parent_child_pair = ();
    delete $atoms_available{$_} for keys %atoms_available;
    foreach my $id (keys %context_dependent_shape_rep) {
        ...
        # my $label_parent = &get_product_label($id_nauo_parent);
        ...
        # %atoms_available
        $atoms_available{$label_parent} = 1;
        $atoms_available{$label_child}  = 1;
    }
}
...
sub step_count_atomic_part {
    ...
    foreach my $parent (keys %assembly) {
        ...
        $atoms_available{$parent} = 1 unless @children;
    }
...
sub read_step_file {
    ...
    # XXX my $file = "../step_data/input/lock_6-pin_assembly_eg.STEP";
    # my $file = "../step_data/input/lock_6-pin_assembly_eg_less_one.STEP";  # problem with count atomic parts
    # my $file = "../step_data/input/lock_assembly_eg.STEP";
...
=dd
sub XXX_to_be_copied_n_deleted {
...
sub XXX_trace_all_entities {
    while ( my ($id, $content) = each %line) {
        &trace_entity($id) unless $content =~ m/^\(/;
    }
}

sub XXX_trace_entity {
    my $id = $_[0];
    my $content = $line{$id};
    my $entity;
    if ($content =~ m/^(\w+)\s*(.*)/) {
...
sub XXX_testing_extract_all_lines {
    my $entity = shift;
    foreach my $id (sort keys %line) {
        my $content = $line{$id};
