...
sub tk_embed_height_0 {
   ...
   print "height 0 = $e\n";
   $array_lookup_id_to_hash{$e}{label} = "zero";
...
sub tk_count_atomic_part {
    ...
    foreach my $atom (@atoms) {
        my $available = &tk_is_atom_available($atom, keys %elements_available);
        #print "$atom ($available) is an atom\n";
    }

    print "tk/step count atomic part ... 2nd set\n";
    foreach my $element (keys %elements_available) {
        my $available = &tk_is_atom_available($element, keys %elements_available);
        my @siblings;
