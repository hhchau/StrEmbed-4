...
sub tk_count_atomic_part {
    ...
    # print "$ref_atoms_available\n";
   # my @atom_available;
    #foreach my $atom (@atoms_available) {
    #}
    #foreach my $ref_atom (@atoms_available) {
    #    my $atom      = ${$ref_atom}[0];
    #    my $available = ${$ref_atom}[1];
    #    my $parent = &is_covered_by($atom);
    #    my $parent_avail = &tk_is_atom_available($atom, @atoms_available);
    #    print "$atom ($available) has parent $parent ($parent_avail)\n";
        #print "   ";
        #my @siblings = &covers(&is_covered_by($atom));
        #foreach my $sibling (@siblings) {
        #    my $available2 = &tk_is_atom_available($sibling, @atoms_available);
        #    print " $sibling ($available2)";
        #}
        #print "\n";
    #}
}
...
sub tk_is_atom_available {
    ...
    my $sibling = shift;
    my @atoms_available = @_;
    while (my ($atom, $available) = each %atoms_available) {
    ...
    foreach my $h (0..$#array_with_coords) {
        ...
        foreach my $j (0..$#elements) {
            ...
            $element_label{$id} = $c -> createText($x2 + $element_radius , $y1 + $element_radius,
                 ...
                 -tags => "element_label",
                 -fill => 'gray50',
            ) if $array_with_coords[$h][$j]{label};    # show label only when it is not a "0"
        }
    }
}
