...
sub replot_hasse {
    ...
    &tk_embed_height_0;
    # okay-ish up to here
    print "okay-ish up to just before &tk_hasse\n";
...
sub tk_embed_height_n {
    ...
    my ($ref_assembly, $ref_has_parent) = &step_produce_assembly_has_parent(@parent_child_pair);
    our %tk_assembly = %$ref_assembly;
    our %tk_has_parent = %$ref_has_parent;
    ...
    # $top_level_assembly = &step_top_level_assembly;  # replaced by find_top_level_assembly
...
sub is_covered_by {
    our %tk_has_parent;
    my $child = shift;
    return $tk_has_parent{$child};
}

sub covers {
    our %tk_assembly;
    my $parent = shift;
    return @{$tk_assembly{$parent}}
}
